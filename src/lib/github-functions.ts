...
/**
 * GitHub Functions for VibeHero
 * Consolidated functions for GitHub operations that can be used in API routes and MCP calls
 */
export class GitHubFunctions {
   ...
   */
  static async getUserInstallations(userId: string): Promise<GitHubInstallationsResponse> {
    try {
   ...
   */
  static async getAppInstallations(): Promise<GitHubInstallationsResponse> {
    try {
   ...
   */
  static async linkProjectToRepository(
    projectId: string, 
    repositoryName: string, 
    installationId: number,
    userId: string
   ...
   */
  static async verifyProjectAccess(projectId: string, userId: string): Promise<boolean> {
    try {
      const project = await prisma.project.findFirst({
        where: {
          id: projectId,
   ...
   */
  static async syncIssueToVibeHero(
    projectId: string,
    issueNumber: number,
    userId: string
   ...
   */
  static async getProjectGitHubStatus(
    projectId: string, 
    userId: string
  ): Promise<{
    success: boolean;
  ...
  static async handleWebhook(
    ...
    try {
      ...
      switch (eventType) {
        case 'issues':
          return await GitHubFunctions.handleIssueWebhook(payload);
        case 'issue_comment':
          return await GitHubFunctions.handleCommentWebhook(payload);
        case 'installation':
          return await GitHubFunctions.handleInstallationWebhook(payload);
        default:
   ...
   */
  static async unlinkProjectFromRepository(
    projectId: string,
    userId: string
  ): Promise<{ success: boolean; error?: string }> {
    try {
   ...
   */
  static async createProjectFromRepository(
    repoName: string,
